...
if (isset($_GET['nik'])) {
	...
	// cek ttd db
	$cekttd = mysqli_query($konek, "select * from data_user where nik='$nik'");
	$dta = mysqli_fetch_array($cekttd);
}
?>
<?php
if (isset($_GET['nik'])) {
...
if (isset($_POST['ubah'])) {
	...
	password='$password',
	hak_akses='$hak_akses',
	ttd='$file_ttd' WHERE nik=$nik";
	$query = mysqli_query($konek, $sql);

	if ($query) {
		copy($_FILES['ttd']['tmp_name'], "../ttd/" . $file_ttd);
